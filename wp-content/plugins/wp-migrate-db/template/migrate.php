...
if ( ! $is_default_profile ) {
	...
	*/
	...
	$loaded_profile = $this->profile->maybe_update_profile( $loaded_profile, $_GET['wpmdb-profile'] );
}

if ( false == $is_default_profile ) {
	$loaded_profile = wp_parse_args( $loaded_profile, $this->profile->default_profile );
}
$loaded_profile     = wp_parse_args( $loaded_profile, $this->profile->checkbox_options );
	...
	<form method="post" id="migrate-form" action="#migrate" enctype="multipart/form-data">
		...
		<div class="option-section">
			...
			<ul class="option-group migrate-selection">
				<li>
					<label for="savefile">
						<input id="savefile" type="radio" value="savefile" name="action"<?php echo ( $loaded_profile['action'] == 'savefile' || ! $this->props->is_pro ) ? ' checked="checked"' : ''; ?> />
		...
		<div class="notification-message error-notice directory-permission-notice inline-message" style="display: none;">
			...
			<p><?php echo esc_html( $this->filesystem->get_upload_info( 'path' ) ); ?></p>
				...
				<table id="find-and-replace-sort" class="clearfix replace-fields">
					...
					<?php if ( $is_default_profile ) : ?>
						...
						<tr class="replace-row">
							...
							<td class="old-replace-col">
								<input type="text" size="40" name="replace_old[]" class="code" id="old-path" placeholder="Old file path" value="<?php echo esc_attr( $this->util->get_absolute_root_file_path() ); ?>" autocomplete="off" />
							</td>
							<td class="arrow-col" title="Copy Find to Replace">
								<span class="right-arrow">&rarr;</span>
							</td>
