 ...
 */
class PHPConsoleHandler extends \FSVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     * @throws Exception
     */
    public function __construct(array $options = array(), \FSVendor\PhpConsole\Connector $connector = null, $level = \FSVendor\Monolog\Logger::DEBUG, $bubble = \true)
    {
        if (!\class_exists('FSVendor\\PhpConsole\\Connector')) {
    ...
    private function initConnector(\FSVendor\PhpConsole\Connector $connector = null)
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \FSVendor\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \FSVendor\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\FSVendor\PhpConsole\Helper::isRegistered()) {
            \FSVendor\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \FSVendor\PhpConsole\Handler::getInstance();
    ...
    {
        if ($record['level'] < \FSVendor\Monolog\Logger::NOTICE) {
    ...
    {
        return new \FSVendor\Monolog\Formatter\LineFormatter('%message%');
    }
}
