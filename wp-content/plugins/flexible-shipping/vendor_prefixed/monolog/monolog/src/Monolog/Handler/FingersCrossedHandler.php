...
use FSVendor\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy;
 ...
 */
class FingersCrossedHandler extends \FSVendor\Monolog\Handler\AbstractHandler
{
    protected $handler;
    ...
    public function __construct($handler, $activationStrategy = null, $bufferSize = 0, $bubble = \true, $stopBuffering = \true, $passthruLevel = null)
    {
        if (null === $activationStrategy) {
            $activationStrategy = new \FSVendor\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy(\FSVendor\Monolog\Logger::WARNING);
        }
        // convert simple int activationStrategy to an object
        if (!$activationStrategy instanceof \FSVendor\Monolog\Handler\FingersCrossed\ActivationStrategyInterface) {
            $activationStrategy = new \FSVendor\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy($activationStrategy);
        }
        ...
        if ($passthruLevel !== null) {
            $this->passthruLevel = \FSVendor\Monolog\Logger::toMonologLevel($passthruLevel);
        }
