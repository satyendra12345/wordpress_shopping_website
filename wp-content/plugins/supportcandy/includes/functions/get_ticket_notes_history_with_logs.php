...
$ticket_notes_history_with_logs = '';
...
if($threads){
  ...
  foreach ($threads as $thread) {
    ...
    $ticket_notes_history_with_logs .= '<hr><strong>'.$user_name.'</strong> <small><i>'.$wpscfunction->time_elapsed_timestamp($thread->post_date).'</i> ' . $thread_type  . ' </small><br>'.$thread->post_content;
    
    if($attachments){
      $ticket_notes_history_with_logs .= 'Attachments :';		
      foreach( $attachments as $attachment ):
        ...
        $ticket_notes_history_with_logs.= '<br><a  style="text-decoration:none;" href="'.$download_url.'" target="_blank">'.$attach['filename'].'</a>';
      endforeach;
    }
  }
}
