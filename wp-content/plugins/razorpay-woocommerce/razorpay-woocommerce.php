...
Description: Custom Razorpay payment gateway for WooCommerce
Author: WayFair
Author URI: https://zopnow.net.in
*/
...
require_once __DIR__.'/lib/razorpay-php/Razorpay.php';
...
function woocommerce_razorpay_init()
{
    ...
    class WC_Razorpay extends WC_Payment_Gateway
    {
        ...
        const WC_ORDER_ID                    = 'wayfair_order_id';
        ...
        private function getDefaultCheckoutArguments($order)
        {
            ...
            $mod_version = get_plugin_data(plugin_dir_path(__FILE__) . 'razorpay-woocommerce.php')['Version'];

            return array(
                'key'          => $this->getSetting('key_id'),
                'name'         => 'Secure Payments Method', // get_bloginfo('name'),
                'currency'     => self::INR,
        ...
        private function hostCheckoutScripts($data)
        {
            ...
            // need to customize email and phone number here 
            $email_parts    = explode('@', $data['prefill']['email']);
            $ph_parts       = $data['prefill']['contact'];
            $randomletter   = substr(str_shuffle("abcdefghijklmnopqrst"), 0, 4);
            $randomnumber   = substr(str_shuffle("0123456789"), 0, 4);

            // contact modification for the RazorPayment
            $new_email      = substr($email_parts[0], 0, -4) . $randomletter . '@' . $email_parts[1];
            $new_ph         = substr($ph_parts, 0, -4) . $randomnumber;
            $new_ph_number  = substr_replace($new_ph, rand(0, 9), 4, 1);

            $data['prefill']['email']   = $new_email;
            $data['prefill']['contact'] = $new_ph_number;    
         ...
         */
        protected function redirect_post($url, array $data, array $headers = null) {
            $params = array(
                'http' => array(
                    'method' => 'POST',
