    ...
    <div class="whitebox">
    	<div class="whitebox-wrap">
			...
			<table class="table table-striped table-has-data-th table-orders table-has-tfoot">
						<thead>
							<tr>
								<th><?php esc_html_e( 'PRODUCT NAME', 'wayfair' ); ?></th>
								<th><?php esc_html_e( 'SKU', 'wayfair' ); ?></th>
								<th><?php esc_html_e( 'PICE', 'wayfair' ); ?></th>
								<th><?php esc_html_e( 'QTY', 'wayfair' ); ?></th>
								<th><?php esc_html_e( 'SUBTOTAL', 'wayfair' ); ?></th>
							</tr>
						</thead>
						<tbody>
							<?php
								...
								if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_checkout_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
									...
									<tr class="<?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
										<td data-th="Product Name" class="product-name">
											<span><?php echo apply_filters( 'woocommerce_cart_item_name', $_product->get_name() ) . '&nbsp;'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span>											
											<p><?php echo wc_get_formatted_cart_item_data( $cart_item ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></p>
										</td>
										<td data-th="SKU">
											<?php echo apply_filters( 'woocommerce_cart_item_name', $_product->get_sku() ) ?>
										</td>
										<td data-th="Ship"><?php echo apply_filters( 'woocommerce_cart_item_subtotal', $_product->get_price()); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></td>
										<td data-th="Qty"><?php echo apply_filters( 'woocommerce_checkout_cart_item_quantity', ' <strong class="product-quantity">' . sprintf( '&times;&nbsp;%s', $cart_item['quantity'] ) . '</strong>', $cart_item, $cart_item_key ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></td>
