		...
		<div class="container">
			...
			<div class="frntre-columns-row">
				...
				<div class="frntre-back-link">
					...
					</svg><?php echo _e('Continue Shopping') ?>
					</a>
				</div>
				<div class="page-title"><?php echo _e('My Cart') ?></div>
				<!-- <div class="whitebox">
				...
				<?php foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) { ?>
					...
					// echo '<pre>'; print_r($_product->get_regular_price()); die;
					...
					<?php if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) { ?>
						...
						<div class="whitebox">
							...
							<div class="whitebox-wrap cart-item">
								<div class="discount-label"><?php echo sprintf('%d %% OFF', $discount) ?></div>
								<div class="frntre-image">
									...
									<?php $thumbnail = "<img src='" . get_product_image_url($product_id, 150, 150) . "' alt='" . $_product->get_name() . "'>"; ?>
									<?php
									if ( ! $product_permalink ) {
										echo $thumbnail; // PHPCS: XSS ok.
									?>
									<!-- <img src="<?php echo get_product_image_url($product_id, 300, 300) ?>" alt="<?php echo $_product->get_name() ?>"> -->
									<?php
									} else {
										echo $thumbnail; // PHPCS: XSS ok.
									}
				...
				<div class="frntre-primary">
					<div class="whitebox">
						...
						<div class="qualifies-order">
							...
							<?php if ( WC()->cart->get_cart_contents_total() < 399 ): ?>
								<span><?php echo _e('Minimum Order Rs 399 We Are Serving'); ?></span>
							<?php elseif ( (WC()->cart->get_cart_contents_total() >= 399) && (WC()->cart->get_cart_contents_total() <= 599) ): ?>
								<?php echo sprintf("%s", _e('Add items worth ₹' . (599 - floatval(WC()->cart->get_cart_contents_total())) . ' more for FREE delivery')); ?>
							<?php else: ?>
					...
					<div class="add-card">
						...
						<input type="text" name="AddCard" placeholder="Add Gift Card/Promo Code" class="form-control" id="AddCardPromoCode">
