...
const SERVING_AREAS = [1,2,3,4,5,6,7,8];	
...
//  };
  ...
  $('[id^=billing_postcode]').keypress(validateNumber);
  ...
  if (typeof $('#CheckoutForm').accWizard != 'undefined') {
    ...
    $('#CheckoutForm').accWizard ({
      autoButtonsNextClass: 'btn btn-dark place_order_woo',
      ...
      beforeNextStep: function( currentStep ) {
        ...
        $(block).find('input, textarea').each(function(index, element) {
          ...
          if ( !$(element).is(':hidden') ) {
            ...
            if ( $(element).val().length === 0 ) {
              ...
              // pincode validation
              if ( currentStep === 1 && $(element).attr('id') === 'billing_postcode' ) {

                if (validate_pincode(element)) {
              ...
              if ( !isNaN($(this).find('#DetectCity').val()) ) {
                ...
                if ( jQuery.inArray(pin, SERVING_AREAS) > 0 ) {
                  ...
                  jQuery.ajax({
                    ...
                    data: {action : 'check_add_user_location_cookie', pin: $(this).find('#DetectCity').val()},
                    success: function (data) {
                      ...
                      if ( data.redirect ) {
                        ...
                        $(this).find('.search-results').html(data.html);
...
function validate_pincode(element)	
{	
  if ( !isNaN($(element).val().substr(0, 1)) ) {	
    var pin = eval($(element).val().substr(0, 1));	
    if ( $(element).val().length === 6 ) {	
      if ( !isNaN($(element).val()) ) {	
        if (SERVING_AREAS.includes(pin) > 0) {	
          return true;	
        } else {        	
          swal('We dont server to your area yet');	
          return false;	
        }	
      }	
    }	
...
function displa_variant_price(data, element, container = 'list')
{
  if ( container === 'list' ) {
    ...
    var html = '<span class="price">' +
                '<del><span class="woocommerce-Price-amount amount"><span class="woocommerce-Price-currencySymbol aaaa">M.R.P.&nbsp;</span>' + data.regular_price + '</span></del>' +
                '<ins><span class="woocommerce-Price-amount amount"><span class="woocommerce-Price-currencySymbol bbbbb">' + data.currency + '</span>' + data.display_price + '</span></ins>' +
              '</span>';
    ...
    var html = '<span class="price">' +
                '<del><span class="woocommerce-Price-amount amount"><span class="woocommerce-Price-currencySymbol cccc">M.R.P.&nbsp;</span>' + data.regular_price + '</span></del>' +
                '<ins><span class="woocommerce-Price-amount amount"><span class="woocommerce-Price-currencySymbol dddd">' + data.currency + '</span>' + data.display_price + '</span></ins>' +
              '</span>';
